...
// Detect leading indentation; warn/error on mixed tabs/spaces on a line.
...
func (s *Scanner) measureIndentAndComment() (indent int, isCommentLine bool) {
  ...
  for off := s.i; off < len(s.src); {
    ...
    if r == '#' {
      ...
      // comment-only (ignore), but do NOT warn about mixed indent on comment lines
      if r2 != '{' {
        s.i = off
        s.col = indent + 1
        s.atBOL = false
    ...
    if hadSpace && hadTab {
      s.addErr("lexer.mixed_indentation", "mixed tabs and spaces in indentation", s.line, 1)
    }
    s.i = off
    s.col = indent + 1
    s.atBOL = false
