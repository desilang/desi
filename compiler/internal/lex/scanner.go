...
type ScanError struct {
	CodePath string // e.g., "lexer.unterminated_string" or "lexer.generic_lexer_error"
	Message  string
	File     string
	Line     int
	Col      int
...
func (s *Scanner) Next() Item {
	...
	if s.peekIs('.') && unicode.IsDigit(s.peekRuneN(1)) {
		...
		if n, ok := s.advanceDigitsSep(); n == 0 || !ok {
			s.addErr("lexer.generic_lexer_error", "invalid float fraction", s.line, startCol)
		...
		if s.peekIs('e') || s.peekIs('E') {
			...
			if n, ok := s.advanceDigitsSep(); n == 0 || !ok {
				s.addErr("lexer.generic_lexer_error", "invalid float exponent", s.line, startCol)
		...
		if unicode.IsDigit(r) {
			...
			if r == '0' && s.i+w < len(s.src) {
				...
				case 'x', 'X':
					...
					//   INT_HEX:       0x[H][_H]*
					//   HEX FLOAT:     0x[H][_H]*('.'[H][_H]*)? [pP] [+-]? [D][_D]*
					//   Or             0x('.'[H][_H]+) [pP] [+-]? [D][_D]*
					s.i += w + w2
					s.col += 2
					...
					if !okInt {
						s.addErr("lexer.generic_lexer_error", "invalid hex literal", s.line, startCol)
					...
					if s.peekIs('.') && isHexDigit(s.peekRuneN(1)) {
						...
						if nFrac, okFrac := s.advanceWhileSep(isHexDigit); nFrac == 0 || !okFrac {
							s.addErr("lexer.generic_lexer_error", "invalid hex fraction", s.line, startCol)
					...
					if s.peekIs('p') || s.peekIs('P') {
						...
						if nExp, ok := s.advanceDigitsSep(); nExp == 0 || !ok {
							s.addErr("lexer.generic_lexer_error", "invalid hex float exponent", s.line, startCol)
						...
						if sawFrac {
							s.addErr("lexer.generic_lexer_error", "hex float missing exponent 'p'", s.line, startCol)
				...
				case 'b', 'B':
					...
					if n, ok := s.advanceWhileSep(isBinDigit); n == 0 || !ok {
						s.addErr("lexer.generic_lexer_error", "invalid binary literal", s.line, startCol)
				...
				case 'o', 'O':
					...
					if n, ok := s.advanceWhileSep(isOctDigit); n == 0 || !ok {
						s.addErr("lexer.generic_lexer_error", "invalid octal literal", s.line, startCol)
			...
			if dcount == 0 || !ok {
				s.addErr("lexer.generic_lexer_error", "invalid number", s.line, startCol)
