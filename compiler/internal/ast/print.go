...
// Short strings are summarized as Str, except for match arm results where we
// print Str("...") for readability in tests.
...
type pp struct {
	w              io.Writer
	showStrLiteral bool // when true, print short string literal as Str("...")
}
...
func (p pp) node(n Node, d int) {
	...
	case *FuncDecl:
		// decorators before the header at the same indent as the header
	...
	case *MatchStmt:
		...
		for _, arm := range n.Arms {
			...
			p.wr(": ")
			if arm.Result != nil {
				// For tests, show placeholder text for short strings in match arm results.
	...
	case *StrLit:
		...
		if p.showStrLiteral {
			// For readability we show a placeholder short string.
			p.wr(`Str("...")`)
		} else {
			// summarized form used by most golden AST tests
			p.wr("Str")
		}
