...
func BuildAndRunJSON(entryFile string, keepTmp, verbose bool) ([]byte, error) {
  ...
  sig := sha256Sum(
    "parsebridge-v2",             // tag to invalidate older cache strategies
    runtime.GOOS, runtime.GOARCH, // platform
    string(wrapper),
    string(devParserSrc), // parser module content
    string(devLexerSrc),  // lexer module content (may be empty if missing)
  ...
  // Mirror parser under workDir/compiler/desi/parser.desi
  ...
  // Mirror lexer if available under workDir/compiler/desi/lexer.desi
