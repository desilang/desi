...
func BuildAndRunJSON(entryFile string, keepTmp, verbose bool) ([]byte, error) {
	// 1) Read user entry path (just for embedding the path string)
	entryAbs, err := filepath.Abs(entryFile)
	if err != nil {
		return nil, fmt.Errorf("abs(%s): %v", entryFile, err)
	}

	// 2) Locate dev parser (examples/compiler/desi/parser.desi)
	...
	// 3) Runtime C sources (included in cache signature)
	...
	// 7) Work dir = cache dir (preserve for caching)
	workDir := cacheRoot
