...
func ScanUnusedAndDuplicateImports(src string, imps []ImportDecl) []diag.Diagnostic {
	...
	// (Very lightweight: just strips // and /* */ comments and normalizes whitespace.)
...
func lookup(domain, key, fallbackID, fallbackTitle string) diag.Diagnostic {
	...
	if info, ok := diag.LookupFull(domain, key); ok {
		...
		// Your Diagnostic has no Help field; surface catalog help as a note if present.
...
func withSpan(d *diag.Diagnostic, start, end int) {
	if start <= 0 && end <= 0 {
		return
	}
	// If your diag.Span uses file/line/col rather than byte offsets, adapt here.
	// To avoid tight coupling, we skip attaching spans by default for now.
	// Notes already provide enough context for these warnings.
}

// ----- Optional helper: regex-only import extraction (for integrations without AST) -----
// If you want, callers can use this to build ImportDecls straight from source text.
var (
