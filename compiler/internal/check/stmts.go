...
func (c *checker) checkLet(st *ast.LetStmt) {
  ...
  for i := 0; i < _max; i++ {
    ...
    if declText != "" {
      if want == KindStruct {
        // For now: allow any RHS kind (refined when struct values exist).
        kind = KindStruct
  ...
  // Optional group type currently informational.
  if strings.TrimSpace(st.GroupType) != "" {
    // future: check tuple type vs RHS
  }
}
...
func (c *checker) checkAssign(st *ast.AssignStmt) {
      ...
      case *ast.FieldExpr:
        ...
        for j := 0; j < len(path)-1; j++ {
          ...
          if !ok {
            ...
            current = "" // bail but try to continue
            break
          }
  ...
  for i := 0; i < _max; i++ {
    ...
    // Struct variables: only allow assigning from another var of same struct type
    if v.kind == KindStruct {
      rhsStruct := c.structNameOfExpr(st.Exprs[i])
      if rhsStruct != "" && rhsStruct == v.structName {
        v.written = true
...
func (c *checker) structNameOfExpr(e ast.Expr) string {
  ...
  case *ast.StructLit: // when struct literals exist, carry the name
    return v.Name
  }
  return ""
}
