...
func checkFunc(info *Info, fn *ast.FuncDecl) []error {
  ...
  // body (with branch scopes for if/elif/else)
...
func (c *checker) checkStmt(s ast.Stmt) {
  ...
  case *ast.LetStmt:
    ...
    if k == KindUnknown {
      // leave as unknown; may get refined by later assigns, but keep declared kind
      k = KindUnknown
    }
...
func (c *checker) rebindCurrent(name string, v varInfo) {
  ...
  // If not in current scope, walk up to the defining scope
...
func (c *checker) kindOfExpr(e ast.Expr) Kind {
  ...
  case *ast.IdentExpr:
    ...
    // could be a function name in a call context; alone it's unknown
  ...
  case *ast.BinaryExpr:
    ...
    switch v.Op {
    case "+": // int + int -> int ; str + str|int|bool -> str (very permissive for Stage-0)
      ...
      return KindUnknown
    case "-", "*", "/", "%", "<", "<=", ">", ">=", "==", "!=":
      // treat as int-ish (booleans flow to int in codegen)
      if _, ok := unifyKinds(lk, rk); ok {
        return KindInt
      }
      return KindUnknown
    case "and", "or", "|>":
      // keep it int-ish for now
      return KindInt
    default:
      return KindUnknown
    }
  ...
  case *ast.FieldExpr:
    // possibly io.println callee; as a value it's unknown
    return KindUnknown

  case *ast.IndexExpr:
    // not supported in Stage-0 type-wise
    return KindUnknown
  ...
  case *ast.CallExpr:
    ...
    // calling non-ident callee (e.g., (f)())
    return KindUnknown
...
func mapTextType(t string) Kind {
  ...
  case "str", "string":
    return KindStr
  default:
    // Stage-0: unknown/opaque types treated as int slots in codegen,
    // but type checker keeps them unknown to catch misuse where possible.
    return KindUnknown
  }
}
...
func unifyKinds(a, b Kind) (Kind, bool) {
  ...
  // allow int<->bool in Stage-0 (both lower to int in codegen)
