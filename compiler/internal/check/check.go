...
func Check(mod *ast.Module) *Result {
  ...
  // 1) Resolve imports up-front (Phase-1). Use in-memory loader by default;
  // CLI will use FSLoader. We merge resolve diags and inject names.
  rdiags, rinfo := resolve.Resolve(mod, resolve.NewMemLoader(nil))
  for _, d := range rdiags {
    res.Diags = append(res.Diags, d)
  }
  injectImports(NewScope(nil), rinfo) // ensure injected into the top scope we create below

  // 2) Build top scope and walk (existing behavior)
  top := NewScope(nil)
  res.Info.Top = top
