...
func (c *checker) kindOfExpr(e ast.Expr) Kind {
  ...
  case *ast.FieldExpr:
    // Handle nested chains like a.b.c: mark base as read and walk struct types.
    ...
    for i, seg := range path {
      ...
      // If not last hop, must be struct to continue chaining.
      if i < len(path)-1 {
  ...
  case *ast.CallExpr:
    ...
    // builtin print(...) — works without imports and mirrors io.println
