...
func (c *checker) typBinary(x *ast.BinaryExpr) types.T {
  ...
  case "+", "-", "*", "/", "%", "**":
    ...
    if op == "+" && (types.Equal(lt, types.Str) || types.Equal(rt, types.Str)) {
      ...
      // Allow coercion for core primitives: int/float/bool/str
      if types.Equal(other, types.Int) || types.Equal(other, types.Float) ||
        types.Equal(other, types.Bool) || types.Equal(other, types.Str) {
        c.info.Types[x] = types.Str
        return types.Str
      }
      // Unknown/unsupported other side — fall through to numeric rules below,
      // which will ultimately error out (we don't invent conversions).
    }
  ...
  case "^": // exponent is POW ("**"); "^" is bitwise XOR
    lt := c.typ(x.Lhs)
    rt := c.typ(x.Rhs)
    ...
    c.add(diagAt("DTE0004", x.Span, "invalid operands for '^'"))
    return nil
  ...
  case "|>":
    ...
    // rewrite: new args = [lhs] + args
    args := make([]ast.Expr, 0, 1+len(call.Args))
    args = append(args, x.Lhs)
    args = append(args, call.Args...)
    call2 := &ast.CallExpr{Callee: call.Callee, Args: args, Span: call.Span}
    return c.typ(call2)
