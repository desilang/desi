...
func CheckFile(f *ast.File) (*Info, []error, []Warning) {
	...
	// ...
	// [KEEP THE REST OF THE FILE EXACTLY AS YOU POSTED AFTER THIS POINT]
	// (No additional changes beyond calling scanForVoidTypes above.)
...
func scanForVoidTypes(f *ast.File) []error {
	...
	for _, d := range f.Decls {
		...
		case *ast.ConstDecl:
			...
			// Example (uncomment if field exists):
