...
func EmitFile(f *ast.File, info *check.Info) string {
	var b bytes.Buffer
	term.Wprintf(&b, "/* generated by desic (Stage-1 kickoff) */\n")
...
func emitStmt(b *bytes.Buffer, indent int, s ast.Stmt, e *env) {
	...
	case *ast.ReturnStmt:
		// Before returning, run defers in LIFO
		if len(e.defers) > 0 {
	...
	case *ast.DeferStmt:
		// Stage-0/1: record function-scope defers (must be call expr, per checker)
...
func cExprFor(e ast.Expr, env *env) (string, string) {
	...
	case *ast.BinaryExpr:
		...
		// String concatenation lowering: a + b  -> desi_str_concat(a, b)
		...
		k := ""
		if lk == "str" || rk == "str" {
			// Only '+' is meaningful for strings; other ops default to int in checker.
			k = "str"
