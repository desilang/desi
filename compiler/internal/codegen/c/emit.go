...
func EmitFile(f *ast.File, info *check.Info) string {
	...
	// Validate types supported by the C backend.
	if verrs := validateTypes(f); len(verrs) > 0 {
		term.Wprintf(&b, "/* C backend type validation errors: */\n")
		for _, msg := range verrs {
			term.Wprintf(&b, "#error desic: %s\n", msg)
		}
		term.Wprintf(&b, "\n")
	}
