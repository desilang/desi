...
func emitStmt(b *bytes.Buffer, indent int, s ast.Stmt, e *env) {
  ...
  case *ast.MatchStmt:
    // Lower to: { <EnumType> __scrut = <expr>; switch (__scrut.tag) { ... } }
    cx, ck := cExprFor(st.Scrut, e)
    if !strings.HasPrefix(ck, "enum:") {
      term.Wprintf(b, "%s/* match scrutinee is not enum at codegen */ (void)(%s);\n", ind, cx)
      break
    }
    enumName := strings.TrimSpace(ck[len("enum:"):])
    term.Wprintf(b, "%s%s __scrut = %s;\n", ind, cTypeFromText(enumName, e.info), cx)
    term.Wprintf(b, "%sswitch (__scrut.tag) {\n", ind)
    for _, arm := range st.Arms {
      term.Wprintf(b, "%scase %s_%s: {\n", ind, enumName, arm.Pat.Variant)
      // Optional payload binding
      if arm.Pat.Bind != "" {
        if ei, ok := e.info.Enums[enumName]; ok {
          if payload, ok2 := ei.Variants[arm.Pat.Variant]; ok2 && !isNoneText(payload) {
            term.Wprintf(b, "%s  %s %s = __scrut.as.%s;\n",
              ind, cTypeFromText(payload, e.info), arm.Pat.Bind, arm.Pat.Variant)
            e.vars[arm.Pat.Bind] = payload
          }
        }
      }
