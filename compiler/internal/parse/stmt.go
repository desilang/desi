...
func (p *Parser) parseBlock() *ast.Block {
  ...
  // Docstring: if the *first* statement is a bare string, convert it
  // into a DocStringStmt node. (Attachment to Func/Class happens later.)
  if len(blk.Stmts) > 0 {
    if es, ok := blk.Stmts[0].(*ast.ExprStmt); ok {
      if lit, ok2 := es.Expr.(*ast.StrLit); ok2 {
