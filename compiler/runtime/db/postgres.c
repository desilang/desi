...
// Cache up to PG_PREP_CACHE_SIZE named prepared statements per connection.
// Uses FNV-1a hash of the SQL string for fast lookup.
// LRU eviction: when cache is full, the least-recently-used entry is
// deallocated on the server via Close('S', stmt_name) and replaced.
#define PG_PREP_CACHE_SIZE 64
#define PG_PREP_NAME_LEN   16  // "ps_XXXXXXXX\0" = 12 chars + padding

typedef struct {
  uint64_t sql_hash;                    // FNV-1a hash of the SQL query
  char     stmt_name[PG_PREP_NAME_LEN]; // server-side statement name
  uint64_t lru_tick;                    // last-used timestamp (monotonic counter)
...
typedef struct {
  ...
  // Prepared statement cache
  PGPrepEntry prep_cache[PG_PREP_CACHE_SIZE];
  int         prep_count;    // number of occupied slots
  uint64_t    prep_lru_tick; // monotonic counter for LRU ordering
  uint32_t    prep_seq;      // sequence number for unique stmt names
} PGConn;
...
// ============================================================
// Prepared Statement Cache — FNV-1a hash + helpers
// ============================================================

// FNV-1a 64-bit hash
static uint64_t pg_fnv1a(const char *s) {
...
// Lookup a cached prepared statement by SQL hash.
// Returns slot index or -1 if not found.
static int pg_prep_lookup(uint64_t hash) {
  if (!g_pg) return -1;
  for (int i = 0; i < PG_PREP_CACHE_SIZE; i++) {
    if (g_pg->prep_cache[i].in_use && g_pg->prep_cache[i].sql_hash == hash) {
      // Update LRU tick
      g_pg->prep_cache[i].lru_tick = ++g_pg->prep_lru_tick;
      return i;
    }
  }
  return -1;
...
// Find a free slot, or return the LRU slot index for eviction.
static int pg_prep_find_slot(void) {
  if (!g_pg) return 0;
  // First, look for an empty slot
  for (int i = 0; i < PG_PREP_CACHE_SIZE; i++) {
    if (!g_pg->prep_cache[i].in_use) return i;
  }
  // All full — find LRU (smallest lru_tick)
  int lru = 0;
  for (int i = 1; i < PG_PREP_CACHE_SIZE; i++) {
    if (g_pg->prep_cache[i].lru_tick < g_pg->prep_cache[lru].lru_tick)
      lru = i;
  }
  return lru;
}

// Forward decl — pg_send_raw is defined below
static int pg_send_raw(const char *data, int len);
static int pg_read_msg(char *out_type, char *payload, int *out_plen);

// Send Close('S', stmt_name) + Sync to deallocate a named statement.
// We must drain the response (CloseComplete + ReadyForQuery).
static void pg_prep_close_stmt(const char *stmt_name) {
  if (!g_pg || g_pg->fd < 0) return;
  char buf[256];
  int pos = 0;
  int slen = strlen(stmt_name);
  // Close message: 'C' + int32(len) + 'S' + cstring(name)
  buf[pos++] = 'C';
  pg_write_i32(buf + pos, 4 + 1 + slen + 1); pos += 4;
  buf[pos++] = 'S';
  memcpy(buf + pos, stmt_name, slen + 1); pos += slen + 1;
  // Sync
  buf[pos++] = 'S';
  pg_write_i32(buf + pos, 4); pos += 4;
  pg_send_raw(buf, pos);
  // Drain responses until ReadyForQuery
  char mtype; char drain_buf[PG_BUF_SIZE]; int plen;
  for (int i = 0; i < 10; i++) {
    if (pg_read_msg(&mtype, drain_buf, &plen) < 0) break;
    if (mtype == 'Z') break;
  }
}

// Evict a cache slot (deallocate on server, mark free)
static void pg_prep_evict(int slot) {
  if (!g_pg || !g_pg->prep_cache[slot].in_use) return;
  pg_log("prep_cache: evicting slot %d stmt=%s", slot,
         g_pg->prep_cache[slot].stmt_name);
  pg_prep_close_stmt(g_pg->prep_cache[slot].stmt_name);
  g_pg->prep_cache[slot].in_use = 0;
  g_pg->prep_count--;
}

// Insert a new entry into the cache (evicts LRU if needed)
static int pg_prep_insert(uint64_t hash, const char *stmt_name) {
  if (!g_pg) return -1;
  int slot = pg_prep_find_slot();
  if (g_pg->prep_cache[slot].in_use) {
    pg_prep_evict(slot);
  }
  g_pg->prep_cache[slot].sql_hash = hash;
  strncpy(g_pg->prep_cache[slot].stmt_name, stmt_name, PG_PREP_NAME_LEN - 1);
  g_pg->prep_cache[slot].stmt_name[PG_PREP_NAME_LEN - 1] = '\0';
  g_pg->prep_cache[slot].lru_tick = ++g_pg->prep_lru_tick;
  g_pg->prep_cache[slot].in_use = 1;
  g_pg->prep_count++;
  return slot;
}

// Clear entire cache (no server communication — used on close/reconnect)
static void pg_prep_clear(void) {
  if (!g_pg) return;
  for (int i = 0; i < PG_PREP_CACHE_SIZE; i++) {
    g_pg->prep_cache[i].in_use = 0;
  }
  g_pg->prep_count = 0;
  g_pg->prep_lru_tick = 0;
  // Don't reset prep_seq — keep names globally unique across reconnects
  pg_log("prep_cache: cleared");
}

// Public API: clear prepared statement cache
int32_t __pg_clear_prep_cache(void) {
  if (!g_pg) return -1;
  // Deallocate each stmt on the server if still connected
  if (g_pg->connected && g_pg->fd >= 0) {
    for (int i = 0; i < PG_PREP_CACHE_SIZE; i++) {
      if (g_pg->prep_cache[i].in_use) {
        pg_prep_close_stmt(g_pg->prep_cache[i].stmt_name);
      }
    }
  }
  pg_prep_clear();
...
static int pg_read_msg(char *out_type, char *payload, int *out_plen) {
  if (pg_recv_raw(out_type, 1) < 0)
    return -1;
  char lb[4];
  if (pg_recv_raw(lb, 4) < 0)
    return -1;
...
static void pg_md5_block(uint32_t st[4], const unsigned char blk[64]) {
  ...
  for (int i = 0; i < 64; i++) {
    ...
    d = c;
    c = b;
    b = b + PG_ROTL32(a + f + pg_md5_T[i] + M[g], pg_md5_s[i]);
    a = t;
  }
  st[0] += a;
  st[1] += b;
...
static void pg_md5(const unsigned char *data, size_t len, char hex[33]) {
  ...
  for (i = 0; i + 64 <= len; i += 64)
    pg_md5_block(st, (const unsigned char *)(data + i));
  size_t rem = len - i;
  memset(buf, 0, 64);
  ...
  uint64_t bits = len * 8;
  for (int j = 0; j < 8; j++)
    buf[56 + j] = (bits >> (j * 8)) & 0xFF;
  pg_md5_block(st, buf);
  unsigned char dig[16];
  for (int j = 0; j < 4; j++) {
    dig[j * 4] = st[j] & 0xFF;
    dig[j * 4 + 1] = (st[j] >> 8) & 0xFF;
    dig[j * 4 + 2] = (st[j] >> 16) & 0xFF;
    dig[j * 4 + 3] = (st[j] >> 24) & 0xFF;
  }
  for (int j = 0; j < 16; j++)
    sprintf(hex + j * 2, "%02x", dig[j]);
  hex[32] = '\0';
}

static void pg_md5_auth(const char *user, const char *pass, const char salt[4],
                        char out[36]) {
...
static void pg_sha256(const unsigned char *data, size_t len,
  ...
  size_t i;
  for (i = 0; i + 64 <= len; i += 64)
    pg_sha256_block(st, (const unsigned char *)(data + i));
  size_t rem = len - i;
  memset(buf, 0, 64);
  memcpy(buf, data + i, rem);
  buf[rem] = 0x80;
  ...
  uint64_t bits = len * 8;
  for (int j = 0; j < 8; j++)
    buf[63 - j] = (bits >> (j * 8)) & 0xFF;
  pg_sha256_block(st, buf);
  for (int j = 0; j < 8; j++) {
    out[j * 4] = (st[j] >> 24) & 0xFF;
    out[j * 4 + 1] = (st[j] >> 16) & 0xFF;
    out[j * 4 + 2] = (st[j] >> 8) & 0xFF;
    out[j * 4 + 3] = st[j] & 0xFF;
  }
}
...
static const char pg_b64_chars[] =
...
static int pg_b64_decode(const char *in, unsigned char *out) {
  ...
  if (!tab_init) {
    memset(tab, 64, 256);
    for (int i = 0; i < 64; i++)
      tab[(unsigned char)pg_b64_chars[i]] = i;
    tab_init = 1;
  }
  ...
  for (int i = 0; i < len; i += 4) {
    ...
    for (int j = 0; j < 4; j++) {
      if (i + j < len && in[i + j] != '=')
        v = (v << 6) | tab[(unsigned char)in[i + j]];
      else {
        v <<= 6;
        pad++;
      }
...
static void pg_xor32(unsigned char *out, const unsigned char *a,
...
static int pg_scram_auth(const char *user, const char *password) {
  ...
  // Build SASLInitialResponse: 'p' + len + "SCRAM-SHA-256\0" + int32(cfm_len) +
  // cfm
  {
    ...
    int mp = 0;
    msg[mp++] = 'p';
    pg_write_i32(msg + mp, body_len);
    mp += 4;
    memcpy(msg + mp, mech, mech_len + 1);
    mp += mech_len + 1;
    pg_write_i32(msg + mp, cfm_len);
    mp += 4;
    memcpy(msg + mp, cfm, cfm_len);
    mp += cfm_len;
    if (pg_send_raw(msg, mp) < 0)
      return -1;
  ...
  if (atype2 != 11) {
    snprintf(g_pg->error, sizeof(g_pg->error),
             "Expected SASLContinue(11), got %d", atype2);
    return -1;
  }
  ...
  if (!server_nonce || !salt_b64) {
    snprintf(g_pg->error, sizeof(g_pg->error),
             "SCRAM: invalid server-first-message");
    return -1;
  }
  ...
  pg_pbkdf2_sha256(password, strlen(password), salt, salt_len, iterations,
                   salted_password);

  // ClientKey = HMAC(SaltedPassword, "Client Key")
  unsigned char client_key[32];
  pg_hmac_sha256(salted_password, 32, (const unsigned char *)"Client Key", 10,
                 client_key);
  ...
  pg_hmac_sha256(salted_password, 32, (const unsigned char *)"Server Key", 10,
                 server_key);
  ...
  // + cfm_wo_proof Reconstruct server-first-message (we null-terminated commas,
  // need original) Re-read sfm from payload
  memcpy(sfm, payload + 4, sfm_len);
  sfm[sfm_len] = '\0';

  char auth_message[4096];
  snprintf(auth_message, sizeof(auth_message), "%s,%s,%s", cfm_bare, sfm,
           cfm_wo_proof);

  // ClientSignature = HMAC(StoredKey, AuthMessage)
  unsigned char client_sig[32];
  pg_hmac_sha256(stored_key, 32, (const unsigned char *)auth_message,
                 strlen(auth_message), client_sig);
  ...
  pg_hmac_sha256(server_key, 32, (const unsigned char *)auth_message,
                 strlen(auth_message), expected_server_sig);
  ...
  // Send SASLResponse: 'p' + len + cfm_final
  {
    ...
    pg_write_i32(msg + mp, body_len);
    mp += 4;
    memcpy(msg + mp, cfm_final, cfm_final_len);
    mp += cfm_final_len;
    if (pg_send_raw(msg, mp) < 0)
      return -1;
    ...
    if (mtype == 'E') {
      ...
      while (p < payload + plen && *p) {
        ...
        if (f == '\0')
          break;
        if (f == 'M') {
          snprintf(g_pg->error, sizeof(g_pg->error), "%s", p);
          break;
        }
        p += strlen(p) + 1;
      }
  ...
  if (atype3 == 12) {
    ...
    snprintf(g_pg->error, sizeof(g_pg->error), "SCRAM: unexpected auth type %d",
             atype3);
    return -1;
  }
  ...
  if (atype3 == 12) {
    ...
    if (mtype == 'R') {
      ...
      if (ok != 0) {
        snprintf(g_pg->error, sizeof(g_pg->error),
                 "SCRAM: auth not OK after final, got %d", ok);
        return -1;
      }
    }
  }
...
static void pg_ensure_cells(int needed) {
  if (needed <= g_pg->cells_cap)
    return;
  int newcap = needed * 2;
  g_pg->cells = realloc(g_pg->cells, newcap * sizeof(char *));
  for (int i = g_pg->cells_cap; i < newcap; i++)
    g_pg->cells[i] = NULL;
...
int32_t __pg_connect(const char *host, int32_t port, const char *dbname,
  ...
  strncpy(g_pg->host, host, sizeof(g_pg->host) - 1);
  g_pg->port = port;
  strncpy(g_pg->dbname, dbname, sizeof(g_pg->dbname) - 1);
  ...
  if (inet_pton(AF_INET, host, &addr.sin_addr) <= 0) {
    ...
    if (!he) {
      snprintf(g_pg->error, sizeof(g_pg->error), "Cannot resolve host: %s",
               host);
      return -1;
    }
  ...
  db_set_read_timeout(g_pg->fd, g_db_read_timeout_ms);
      ...
      if (rd == 1 && ssl_resp == 'S') {
        ...
        if (g_pg->ssl_ctx) {
          ...
          if (SSL_connect(g_pg->ssl) == 1) {
            g_pg->use_ssl = 1;
            pg_log("SSL/TLS connection established (%s)",
                   SSL_get_version(g_pg->ssl));
          } else {
        ...
        pg_log("Server declined SSL (response='%c'), continuing plaintext",
               ssl_resp);
      }
    }
  }
  ...
  pg_write_i32(startup + pos, 196608);
  pos += 4; // version 3.0
  memcpy(startup + pos, "user", 5);
  pos += 5;
  int ul = strlen(user);
  memcpy(startup + pos, user, ul + 1);
  pos += ul + 1;
  memcpy(startup + pos, "database", 9);
  pos += 9;
  int dl = strlen(dbname);
  memcpy(startup + pos, dbname, dl + 1);
  pos += dl + 1;
  memcpy(startup + pos, "client_encoding", 16);
  pos += 16;
  memcpy(startup + pos, "UTF8", 5);
  pos += 5;
  startup[pos++] = '\0';
  ...
  while (1) {
    ...
    if (mtype == 'R') {
      ...
      if (atype == 0) {
        ...
        pg_write_i32(msg + mp, 4 + pwl + 1);
        mp += 4;
        memcpy(msg + mp, password, pwl + 1);
        mp += pwl + 1;
        pg_send_raw(msg, mp);
        ...
        pg_write_i32(msg + mp, 4 + rl + 1);
        mp += 4;
        memcpy(msg + mp, md5res, rl + 1);
        mp += rl + 1;
        pg_send_raw(msg, mp);
...
int32_t __pg_is_connected(void) { return g_pg ? g_pg->connected : 0; }

// Reconnect using stored connection params (called by pool/dispatch on ping
// failure)
int32_t __pg_reconnect(void) {
  if (!g_pg)
    return -1;
  ...
  g_pg->connected = 0;
  // Invalidate prep cache — server-side stmts don't survive reconnect
  pg_prep_clear();
  // Free stale results from previous session
  pg_free_results();
  pg_log("reconnecting to %s:%d db=%s user=%s", g_pg->host, g_pg->port,
         g_pg->dbname, g_pg->user);
  return __pg_connect(g_pg->host, g_pg->port, g_pg->dbname, g_pg->user,
                      g_pg->password);
}

char *__pg_last_error(void) { return g_pg ? strdup(g_pg->error) : strdup(""); }
...
int32_t __pg_query(const char *sql) {
  if (!g_pg || !g_pg->connected || g_pg->fd < 0) {
    if (g_pg)
      snprintf(g_pg->error, sizeof(g_pg->error), "Not connected");
    return -1;
  }
  ...
  while (1) {
    ...
    case 'T': { // RowDescription
      ...
      for (int i = 0; i < nc && i < PG_MAX_COLS; i++) {
        ...
        p += strlen(p) + 1;
        p +=
            18; // tableOID(4)+colAttr(2)+typeOID(4)+typeSize(2)+typeMod(4)+fmtCode(2)
      }
      break;
    }
    case 'D': { // DataRow
    ...
    case 'C': { // CommandComplete
      ...
      if (strncmp(tag, "INSERT", 6) == 0 || strncmp(tag, "UPDATE", 6) == 0 ||
          strncmp(tag, "DELETE", 6) == 0) {
        char *sp = strrchr(tag, ' ');
        if (sp)
          rows_affected = atoi(sp + 1);
      } else if (strncmp(tag, "SELECT", 6) == 0) {
        rows_affected = g_pg->nrows;
      } else if (strncmp(tag, "CREATE", 6) == 0 ||
                 strncmp(tag, "DROP", 4) == 0 ||
                 strncmp(tag, "ALTER", 5) == 0) {
        rows_affected = 0;
      }
      break;
    }
...
int32_t __pg_query_params(const char *sql, const char **params, int nparams) {
  ...
  uint64_t sql_hash = pg_fnv1a(sql);
  int cache_slot = pg_prep_lookup(sql_hash);
  int cache_hit = (cache_slot >= 0);
  char stmt_name[PG_PREP_NAME_LEN];

  if (cache_hit) {
    // Cache hit — reuse existing named statement
    strncpy(stmt_name, g_pg->prep_cache[cache_slot].stmt_name,
            PG_PREP_NAME_LEN);
    pg_log("prep_cache: HIT slot=%d stmt=%s", cache_slot, stmt_name);
  } else {
    // Cache miss — generate a new unique statement name
    snprintf(stmt_name, PG_PREP_NAME_LEN, "ps_%08x", g_pg->prep_seq++);
    pg_log("prep_cache: MISS stmt=%s", stmt_name);
  }

  int stmt_name_len = strlen(stmt_name);
  ...
  // ---- 1. Parse message ('P') — only on cache miss ----
  if (!cache_hit) {
    int sql_len = strlen(sql);
    // stmt_name + query + 0 param types (let server infer)
    int body_len = 4 + (stmt_name_len + 1) + (sql_len + 1) + 2;
    buf[pos++] = 'P';
    pg_write_i32(buf + pos, body_len);
    pos += 4;
    memcpy(buf + pos, stmt_name, stmt_name_len + 1);
    pos += stmt_name_len + 1;
    memcpy(buf + pos, sql, sql_len + 1);
    pos += sql_len + 1;
    buf[pos++] = 0;
    buf[pos++] = 0; // int16(0) = no type OIDs
  }

  // ---- 2. Bind message ('B') ----
  // Format: 'B' + int32(len) + cstring(portal) + cstring(stmt) +
  // int16(num_format_codes) + [int16(fc)...]
  ...
  //         + int16(num_result_format_codes) + [int16(rfc)...]
  {
    ...
    body += 1;    // portal = ""
    body += (stmt_name_len + 1); // stmt = named statement
    ...
    buf[pos++] = 'B';
    pg_write_i32(buf + pos, body);
    pos += 4;
    buf[pos++] = '\0'; // portal = ""
    memcpy(buf + pos, stmt_name, stmt_name_len + 1); // stmt = named
    pos += stmt_name_len + 1;
    ...
    for (int i = 0; i < nparams; i++) {
      if (params[i] == NULL) {
        ...
        pg_write_i32(buf + pos, -1);
        pos += 4;
      } else {
        int plen = strlen(params[i]);
        pg_write_i32(buf + pos, plen);
        pos += 4;
        memcpy(buf + pos, params[i], plen);
        pos += plen;
      }
    }
  ...
  {
    buf[pos++] = 'E';
    pg_write_i32(buf + pos, 4 + 1 + 4);
    pos += 4;          // len = 9
    buf[pos++] = '\0'; // unnamed portal
    pg_write_i32(buf + pos, 0);
    pos += 4; // max rows = 0 (all)
  }

  // ---- 5. Sync message ('S') ----
  {
    buf[pos++] = 'S';
    pg_write_i32(buf + pos, 4);
    pos += 4;
  }
  ...
  while (1) {
    if (pg_read_msg(&mtype, payload, &plen) < 0) {
      snprintf(g_pg->error, sizeof(g_pg->error),
               "Failed to read extended query response");
      return -1;
    }
    ...
    case 'Z': { // ReadyForQuery
      pg_log("ReadyForQuery rows=%d", g_pg->nrows);
      if (g_pg->error[0]) {
        // Don't try to Close a statement that failed to Parse — the server
        // already rolled back the failed statement. Sending Close here risks
        // protocol desync if the statement was never created.
        return -1;
      }
      // On success for a cache miss, insert into the cache.
      // (Cache hits already had their LRU tick updated in pg_prep_lookup.)
      if (!cache_hit) {
        pg_prep_insert(sql_hash, stmt_name);
      }
      return rows_affected;
    }
    default:
...
char *__pg_col_name(int32_t idx) {
  if (!g_pg || idx < 0 || idx >= g_pg->ncols)
    return strdup("");
  return strdup(g_pg->colnames[idx]);
}

char *__pg_get_value(int32_t row, int32_t col) {
  if (!g_pg || row < 0 || row >= g_pg->nrows || col < 0 || col >= g_pg->ncols)
    return strdup("");
  if (!g_pg->cells)
    return strdup("");
...
int32_t __pg_dump_results(void) {
  if (!g_pg) {
    fprintf(stderr, "=== PG: no connection ===\n");
    return 0;
  }
...
char *__pg_connection_info(void) {
  ...
  char buf[512];
  snprintf(buf, sizeof(buf), "fd=%d connected=%d user=%s error=%s", g_pg->fd,
           g_pg->connected, g_pg->user, g_pg->error);
  return strdup(buf);
}
...
//   1. Send Query("COPY table FROM STDIN ...") → server responds with
//   CopyInResponse ('G')
...
int32_t __pg_copy_begin(const char *table_name, const char *columns) {
  ...
  if (columns && columns[0]) {
    snprintf(sql, sizeof(sql), "COPY %s (%s) FROM STDIN WITH (FORMAT text)",
             table_name, columns);
  } else {
    snprintf(sql, sizeof(sql), "COPY %s FROM STDIN WITH (FORMAT text)",
             table_name);
  }
  ...
  snprintf(g_pg->error, sizeof(g_pg->error), "COPY: unexpected response '%c'",
           mtype);
  return -1;
}
...
int32_t __pg_listen(const char *channel) {
  if (!g_pg || !g_pg->connected || !channel)
    return -1;
...
char *__pg_poll_notify(void) {
  ...
  if (mtype == 'A') {
    ...
    pg_log("NOTIFY received: channel=%s payload=%s pid=%d", channel,
           notif_payload, pid);
