  ...
  if len(perr) > 0 {
    ...
    for _, e := range perr {
      ...
      if te, ok := e.(codedWithKey); ok && strings.TrimSpace(te.Code()) != "" {
        ...
        // Also respect Diagnostic.Level so module warnings (DMW…) render as warnings
        isWarn := strings.HasPrefix(codeUp, "DW")
        if dd, ok2 := e.(diag.Diagnostic); ok2 && dd.Level == diag.LevelWarning {
          isWarn = true
        }
