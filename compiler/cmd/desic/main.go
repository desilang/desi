...
func main() {
	...
	case "ast-diff":
		// Require at least two positional args (left and right JSON paths).
		// Flags like --ignore-spans / --ignore-keys=... are handled inside cmdASTDiff.
		if len(os.Args) < 4 {
			term.Eprintln("usage: desic ast-diff [--ignore-spans] [--ignore-keys=k1,k2,...] <left.json> <right.json>")
			os.Exit(2)
		}
		os.Exit(cmdASTDiff(os.Args[2:]))
